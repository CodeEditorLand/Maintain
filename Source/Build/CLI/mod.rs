...
impl Cli {
    ...
    fn execute_build(
        ...
        let env_vars = resolve_environment(profile, config);

        // Apply CLI overrides
        let env_vars = apply_overrides(env_vars, &self.workbench, &self.node_version, &self.environment, &self.dependency);
...
/// Resolve environment variables from profile and config
fn resolve_environment(profile: &Profile, config: &LandConfig) -> HashMap<String, String> {
...
fn execute_build_command(
    ...
    let full_command = if build_args.is_empty() {
        ...
        format!("{} {}", build_command, build_args.join(" "))
    };

    println!("Executing: {}", full_command);

    // Execute build command using standard Rust subprocess
    let mut cmd = std::process::Command::new("pnpm");
    cmd.args(["tauri", "build"]);
    if build_command.to_lowercase().contains("--debug") {
        cmd.arg("--debug");
    }
    ...
    let status = cmd.status()
        .map_err(|e| format!("Failed to execute tauri build: {}", e))?;
