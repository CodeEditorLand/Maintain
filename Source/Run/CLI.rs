...
use crate::Build::Rhai::{load_config, LandConfig, Profile as ConfigProfile};
...
#[clap(
    ...
    long_about = "A configuration-driven run system that enables triggering development \
    runs directly with Cargo instead of shell scripts. Reads configuration \
    from .vscode/land-config.json and supports multiple run profiles with \
    hot-reload support."
)]
pub struct Cli {
    #[clap(subcommand)]
...
fn execute_run_command(
    ...
    let (shell_cmd, args) = cmd_args.split_first()
        .ok_or("Empty command")?;
