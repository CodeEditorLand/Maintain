...
use crate::Run::Definition::Profile;
...
fn merge_shell_env(env: &mut HashMap<String, String>) {
    let relevant_vars = [
        ...
        "RUST_LOG", "AIR_LOG_JSON", "AIR_LOG_FILE", "Level",
...
pub fn is_debug(env: &HashMap<String, String>) -> bool {
    env.get(DebugEnv).map(|v| v == "true").unwrap_or(false)
}
...
pub fn is_watch_enabled(env: &HashMap<String, String>) -> bool {
    env.get(WatchEnv).map(|v| v == "true").unwrap_or(true)
}
...
pub fn format_for_display(env: &HashMap<String, String>) -> String {
    let mut lines: Vec<String> = env.iter()
        .map(|(k, v)| {
            let display_value = if v.is_empty() {
                "(empty)".to_string()
            } else {
                v.clone()
            };
...
pub fn filter_run_vars(env: &HashMap<String, String>) -> HashMap<String, String> {
    let run_prefixes = [
        "NODE_", "HOT_", "WATCH", "LIVE_", "AIR_", "RUST_",
        "Browser", "Bundle", "Clean", "Compile", "Debug",
        "Dependency", "Mountain", "Wind", "Electron", "Level",
    ];

    env.iter()
        .filter(|(k, _)| {
            run_prefixes.iter().any(|prefix| k.starts_with(prefix))
        })
