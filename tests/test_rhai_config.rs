...
fn test_config_loader_get_profile_production() {
	...
	assert!(profile.is_some(), "Production profile should exist in configuration");
...
fn test_config_loader_get_profile_release() {
	...
	assert!(profile.is_some(), "Release profile should exist in configuration");
...
fn test_config_loader_get_profile_nonexistent() {
	...
	assert!(profile.is_none(), "Nonexistent profile should return None");
}
...
fn test_env_var_structure_has_required_keys() {
	...
	use crate::Source::Build::Rhai::{
		ConfigLoader::{get_profile, load},
		ScriptRunner::{ScriptContext, execute_profile_script, load_script},
		create_engine,
	};
	...
	for profile_name in &["debug", "production", "release"] {
		...
		assert!(result.is_ok(), "get_env_vars should be callable in profile '{}'", profile_name);
