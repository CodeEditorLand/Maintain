...
fn load_and_compile_script(engine:&Engine, script_path:&str) -> Result<AST, String> {
	...
	let content = std::fs::read_to_string(script_path).map_err(|e| format!("Failed to read script: {}", e))?;
...
fn execute_get_env_vars(engine:&Engine, ast:&AST) -> Result<HashMap<String, String>, String> {
	...
	match result {
		...
		Err(e) => Err(format!("Failed to execute get_env_vars(): {}", e)),
	}
}
...
fn run_tests() {
	...
	let scripts_available:Vec<_> = config
		.profiles
		.iter()
		.filter(|(_, p)| {
